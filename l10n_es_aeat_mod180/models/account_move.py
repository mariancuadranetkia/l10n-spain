    ...
    def fields_view_get(
        ...
        if move_line_fields:
            ...
            context = move_line_field.attrib.get("context", "{}").replace(
                "{",
                "{'default_move_partner_id': partner_id, ",
                1,
            )
        ...
        for vals in res:
            if vals.get("invl_id"):
                ...
                if invline.informacion_catastral_id:
                    vals[
                        "informacion_catastral_id"
                    ] = invline.informacion_catastral_id.id
        return res
    ...
    def tax_line_move_line_get(self):
        ...
        for tax_line in sorted(self.tax_line_ids, key=lambda x: -x.sequence):
            if tax_line.informacion_catastral_id:
                for dicta in res:
                    if dicta.get("move_tax_line_id", 0) == tax_line.id:
                        dicta.update(
                            {
                                "informacion_catastral_id": tax_line.informacion_catastral_id.id
                            }
                        )
                        break
        return res
