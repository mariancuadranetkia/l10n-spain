...
class InformacionCatastral(models.Model):
    ...
    situacion_inmueble = fields.Selection(
    ...
    calificador_numero = fields.Integer(
        string="Calficador del número", default=0, required=True
    )
        ...
        for info in self:
            if info.cod_postal:
                ...
                if not info.referencia_catastral:
                    ...
                    info.municipio
                    and info.municipio.code
                    and info.municipio.code not in ["NA", "BI", "SS", "VI"]
                ):
                    info.situacion_inmueble = "1"
                elif (
                    info.municipio
                    and info.municipio.code
                    and info.municipio.code in ["BI", "SS", "VI"]
                ):
                    info.situacion_inmueble = "2"
                elif (
                    info.municipio
                    and info.municipio.code
                    and info.municipio.code == "NA"
                ):
